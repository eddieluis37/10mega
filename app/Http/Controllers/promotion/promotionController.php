...
class promotionController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('promotion.index', compact('ventas', 'direccion', 'centros', 'defaultCentro', 'clientes', 'vendedores', 'domiciliarios', 'subcentrodecostos'));
    }
    ...
    public function show()
    {
        ...
        $data = DB::table('promotions as pr')
            ->join('users as user', 'pr.user_id', '=', 'user.id')
            ->select([
                'pr.*',
                'user.name as namethird',
            ])
        ...
        return Datatables::of($data)->addIndexColumn()
            ...
            ->addColumn('action', function ($data) {
                ...
                $btn .= '<a href="promotion/create/' . $data->id . '" class="btn btn-dark" title="Ver Promocion" target="_blank">
    ...
    public function storePromotion(Request $request) // Parrilla-Mostrador
    {
        //   $centros = Centrocosto::WhereIn('id', [1])->get();
        // Obtiene los IDs de los centros de costo asociados a las tiendas del usuario autenticado.
        $centroIds = Auth::user()->stores->pluck('centrocosto_id')->unique();
