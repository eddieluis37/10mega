...
class exportFacturaController extends Controller
{
    public function showFactura($id)
    {
        $sale = Sale::findOrFail($id)
            ...
            ->join('stores as s', 'sales.store_id', '=', 's.id')
            ...
            ->select('sales.*', 'u.name as nameuser', 'third.name as namethird', 'fp.nombre as formapago1', 'fp2.nombre as formapago2', 'fp3.nombre as formapago3', 'third.identification', 'third.direccion', 's.name as namecentrocosto', 'third.porc_descuento', 'sales.total_iva', 'sales.vendedor_id')
            ->where([
                ['sales.id', $id],
