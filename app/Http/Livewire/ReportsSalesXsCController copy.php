...
class ReportsSalesXsCController extends Component
{
    ...
    public $componentName, $data, $details, $sumDetails, $countDetails,
        $reportType, $IdSubcentro, $IdInicial,  $IdFinal, $dateFrom, $dateTo, $saleId;
    ...
    public function render()
    {
        ...
        return view('livewire.reports_sales_x_sc.component', [
            'subcentro' => Subcentrocosto::select('id', 'name')->orderBy('name')->get(),
    ...
    public function SalesByDate()
    {
        ...
        // Filtro opcional por subcentro
        if (!empty($this->IdSubcentro) && $this->IdSubcentro != 0) {
            $query->where('sales.subcentrocostos_id', $this->IdSubcentro);
        }

        // Ejecutar agrupamiento y obtener resultados
        $this->results = $query
            ->groupBy('subcentrocostos.id', 'subcentrocostos.name')
            ->orderBy('subcentrocostos.name', 'ASC')
            ->get();
