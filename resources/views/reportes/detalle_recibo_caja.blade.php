...
<div class="container mx-auto py-6">
  <div class="bg-white shadow rounded-lg overflow-hidden">
    ...
    <div class="flex flex-wrap bg-gray-100 p-4 border-b text-black">
      ...
      <div class="w-1/2 sm:w-1/4 mb-2"><strong>Forma Pago:</strong> {{ optional($recibo->paymentMethod)->nombre }}</div>
    </div>
      ...
      <tbody class="bg-white divide-y divide-gray-100">
        @foreach($recibo->details as $detail)
          @php
            ...
            $clientName = optional(optional($sale)->third)->name;
            $invoiceNo  = $sale->consecutivo ?? $ctc->id;
          @endphp
          <tr>
            <td class="px-4 py-2">{{ $clientName }}</td>
            <td class="px-4 py-2">{{ $invoiceNo }}</td>
            <td class="px-4 py-2 text-right">{{ number_format($detail->vr_deuda, 0, ',', '.') }}</td>
            <td class="px-4 py-2 text-right">{{ number_format($detail->vr_pago, 0, ',', '.') }}</td>
            <td class="px-4 py-2 text-right">{{ number_format($detail->nvo_saldo, 0, ',', '.') }}</td>
          </tr>
        @endforeach
      </tbody>
