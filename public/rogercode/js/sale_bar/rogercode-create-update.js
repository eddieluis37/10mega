...
btnAddVentaDomicilio.addEventListener("click", async (e) => {
    ...
    sendData("/ventasave_bar", dataform, token).then((resp) => {
        console.log(resp);
        if (resp.status == 1) {
            ...
            if (resp.registroId != 0) {
                //for new register
                window.location.href = `sale_bar/create/${resp.registroId}`;
            } else {
                refresh_table();
            }
        }
...
const storeBtn = document.getElementById("storeBarMostradorBtn");
...
storeBtn.addEventListener("click", async (e) => {
  ...
  try {
    const resp = await sendData("/store-bar-mostrador", dataform, token);
    console.log(resp);
